...
export function useVapi(book: IBook) {
    ...
    const handlers = {
      ...
      error: (error: Error) => {
        ...
        if (
          ...
          setLimitError(
            "انتهت الجلسة بسبب عدم النشاط. انقر على الميكروفون للبدء من جديد.",
          );
        } else if (
          ...
          setLimitError(
            "الاتصال مفقود. يرجى التحقق من الإنترنت والمحاولة مرة أخرى.",
          );
        } else {
          setLimitError(
            "انتهت الجلسة بشكل غير متوقع. انقر على الميكروفون للبدء من جديد.",
          );
        }
  ...
  const start = useCallback(async () => {
    if (!userId) {
      setLimitError("يرجى تسجيل الدخول لبدء جلسة صوتية.");
      return;
    }
    ...
    try {
      ...
      if (!result.success) {
        setLimitError(
          result.error ||
            "تم الوصول إلى الحد الأقصى لعدد الجلسات الصوتية. يرجى ترقية خطتك.",
        );
        setStatus("idle");
        return;
      }
